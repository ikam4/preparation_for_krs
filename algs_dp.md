...
Сначала реализуем наивно за O(n*W\*log(W)), но с оптимизацией по памяти, используя только два слоя

Лог появляется из-за того, что мы не используем предметы веса a_i больше чем W / a_i раз.
```cpp
vector<int> dp(W + 1, 0);
for (int i = 0; i < n; ++i) {
...
Если требуется максимизация веса, а не стоймости, можно оптимизировать до O(n*W), используя монотонную очередь
