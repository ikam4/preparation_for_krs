...
Рассмотрим каждый элемент $ x \in S $.  
В паре $ (A, B) $ с $ A \subseteq B \subseteq S $ элемент $ x $ может находиться ровно в одном из трёх состояний:

1. $ x \notin B $ → тогда $ x \notin A $;  
2. $ x \in B $, но $ x \notin A $;  
3. $ x \in A $ → тогда $ x \in B $.
...
$$
\underbrace{3 \cdot 3 \cdot \ldots \cdot 3}_{n\ \text{раз}} = 3^n
$$
